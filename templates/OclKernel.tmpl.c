__kernel void kernel($inputs, __global $dtype $output, __local $d_type block)
{
    ...
    const unsigned int id = $global_index
    ...
    const unsigned int id_b = $local_block_index

    // Copy block to local memory
    block[id_b] = $input_grid[id];
